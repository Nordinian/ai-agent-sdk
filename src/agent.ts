...
export type AgentOptions = {
  ...
  /**
   * Gemini-specific: enable Google Search grounding.
   * The model can search the web to ground its responses with up-to-date information.
   */
  geminiGrounding?: boolean
  /**
   * Gemini-specific: cached content resource name for context caching.
   * Reduces token cost for repeated long contexts (e.g. large codebases).
   */
  geminiCachedContent?: string
  /**
   * Gemini-specific: override thinking token budget.
...
export class Agent {
  ...
  private resolveEnvOptions(): void {
    ...
    if (env.GOOGLE_VERTEX_AI && !process.env.GOOGLE_VERTEX_AI) {
      process.env.GOOGLE_VERTEX_AI = env.GOOGLE_VERTEX_AI
    }
  }
